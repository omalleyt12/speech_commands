...
silence_percentage = 15.0 # what percent of training data should be silence
unknown_percentage = 15.0 # what percent of training data should be unknown words
...
def get_batch(data_index,batch_size,offset=0,mode="train",style="full"):
    ...
    if mode != "comp": # add silence and unknowns to batches randomly
        ...
        for j in range(silence_recs):
            ...
            if pseudo_picker < 0.5:
                ...
                pseudo_index = np.random.randint(0,len(pseudo_silence)-1)
                recs.append(pseudo_silence[pseudo_index]["data"])
                sample_weights.append(pseudo_silence[pseudo_index]["prob"])
        ...
        if style != "full":
            ...
            for j in range(unknown_recs):
                ...
                if pseudo_picker < 0.5:
                    ...
                    pseudo_index = np.random.randint(0,len(pseudo_unknown)-1)
                    recs.append(pseudo_unknown[pseudo_index]["data"])
                    sample_weights.append(pseudo_unknown[pseudo_index]["prob"])
                labels.append(1)
    ...
    if mode != "comp":
        ...
        feed_dict[sample_weights_ph] = np.stack(sample_weights).astype(np.float32)
    return feed_dict
...
sample_weights_ph = tf.placeholder(tf.float32,[None])
...
loss = tf.pow(sample_weights_ph,2)*tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels_ph,logits=final_layer)
