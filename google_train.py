...
for steps, learning_rate in zip(training_step_list,learning_rate_list):
    for i in range(steps): 
        ...
        for j in range(batch_size):
            ...
            data[j,:,:] = sess.run(mfcc,feed_dict=feed_dict)
        ...
        if current_step % eval_step == 0 or current_step == sum_steps:
            ...
            val_data = np.zeros((val_size,mfcc_height,mfcc_width))
            val_labels = np.zeros((val_size,len(wanted_words)))
            for i in range(val_size):
                samp_data = data_index[set_name][i]
                ...
                val_data[i,:,:] = sess.run(mfcc,feed_dict=feed_dict)
                val_labels[i,word2index[samp_data["label"]]] = 1
            val_summary, val_accuracy, val_cem = sess.run(
                [merged_summaries,accuracy,cross_entropy_mean],
                feed_dict={
                    fingerprint_ph: val_data,
...
for wav_path in gfile.Glob(test_dir + "/*.wav"):
    ...
    test_index.append({"data":wav_path,"identifier":file_name})
test_data = np.zeros((1000,mfcc_height,mfcc_width))
test_pred = []
batch_index = 0
for i in range(len(test_index)):
    ...
    feed_dict={
        wav_filename_ph: samp_data["file"],
        volume_ph: 1,
        time_shift_padding_ph: [[0,0],[0,0]],
        time_shift_offset_ph: [0,0],
        bg_ph: np.zeros((16000,1)),
        bg_volume_ph: 0
    }

    test_data[batch_index,:,:] = sess.run(mfcc,feed_dict=feed_dict)
    batch_index += 1
    if batch_index % 1000 == 0 or i == (len(test_index) - 1):
        # predict it baby
        test_pred += sess.run(predicted_indices,
 feed_dict= {
                          fingerprint_ph: test_data,
                          keep_prob:0.5
                      })
...
test_stuff = [{"fname":ti["identifier"],"label":tl} for ti,tl in zip(test_index,test_labels)]

df = pd.DataFrame(test_stuff)
