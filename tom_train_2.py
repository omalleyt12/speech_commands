...
def load_train_data():
    ...
    # only read the wav files from disk once
    for i, rec in enumerate(data_index["train"]):
        data_index["train"][i]["data"] = sess.run(wav_data,feed_dict={wav_in_mem_ph:False,wav_filename_ph: data_index["train"][i]["file"],wav_ph:np.zeros((sample_rate,1))})
...
# placeholder for whether to look for an in-mem wave or not
wav_in_mem_ph = tf.placeholder(tf.bool,[])
wav_ph = tf.placeholder(tf.float32,[sample_rate,1])

volume_ph = tf.placeholder(tf.float32,[])

# read from in-mem wav if exists fs
wav_data = tf.cond(wav_in_mem_ph, lambda: wav_ph, lambda: load_disk(wav_filename_ph))
scaled_wav = tf.multiply(wav_data,volume_ph)
...
def get_mfcc_and_labels(data_index,batch_size,sess,word2index,offset=0,mode="train",return_labels=True):
    ...
    for j in range(batch_size):
        ...
        if mode == "train": # will pull from in-memory data
            feed_dict={
                wav_filename_ph: samp_data["file"],
                wav_ph: samp_data["data"],
                wav_in_mem_ph:True,
                volume_ph: volume,
        ...
        else: # will read directly from file
            feed_dict={
                wav_filename_ph: samp_data["file"],
                wav_ph: np.zeros((sample_rate,1)),
                wav_in_mem_ph:False,
                volume_ph: volume,
                time_shift_padding_ph: time_shift_padding,
                time_shift_offset_ph: time_shift_offset,
